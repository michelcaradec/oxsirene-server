...
{
    internal class ScrapSellerCdiscount : IScrapSeller
    {
        private const string CommercialIDPropertyName = "N° d'Immatriculation";
        private const string CommercialNamePropertyName = "Raison Sociale";

        #region IScrapSeller

        public Uri GetPageUri(string sellerID)
        {
            return new Uri(string.Format(Configuration.Instance["api:scrap_seller:cdiscount:seller_page"], sellerID));
        }

        private static Regex _regex_list
            = new Regex(Configuration.Instance["api:scrap_seller:cdiscount:pattern_list"], RegexOptions.Compiled | RegexOptions.Singleline);
        private static Regex _regex_list_items
            = new Regex(Configuration.Instance["api:scrap_seller:cdiscount:pattern_list_items"], RegexOptions.Compiled);
        private static Regex _regex_clean_html
            = new Regex(Configuration.Instance["api:scrap_seller:cdiscount:pattern_clean_html"], RegexOptions.Compiled);
        ...
        public IDictionary<string, string> GetProperties(string content)
        {
            ...
            if (!string.IsNullOrEmpty(content))
            {
                ...
                if (match.Success)
                {
                    string separator = Configuration.Instance["api:scrap_seller:cdiscount:info_separator"];
